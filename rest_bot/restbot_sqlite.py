...
class DatabaseRestbotHelper:
    def __init__(self, path_to_db="restbot_database.db"):
    ...
    def add_user(self, user_id: int, username: str, status: bool = False):
        sql = ("INSERT INTO Users(id, Username, status) "
               "VALUES (?, ?, ?)")
        parameters = (user_id, username, int(status))
        return self.execute(sql, parameters=parameters, commit=True)

    def update_user(self, user_id: int, username: str, status: bool = False):
        sql = ("UPDATE Users SET (Username, status) = "
               "(?, ?) WHERE id = ?")
        parameters = (username, int(status), user_id)
        return self.execute(sql, parameters=parameters, commit=True)
